...
d.lfc_annot.cv <- d.lfc_annot %>% 
  ...
  group_by(pgRNA_id) %>%
  summarise(mean_lfc = mean(lfc_late_vs_T0, na.rm = TRUE),
            sd_lfc = sd(lfc_late_vs_T0, na.rm = TRUE),
            .groups = "drop")
...
d.nnmd.avg.pgrna <- d.lfc_annot %>%
  group_by( pgRNA_id) %>%
  summarize(lfc_avg = mean(lfc_late_vs_T0, na.rm = TRUE),
...
##rep correlation after normalizing
dim(d.lfc_annot)
...
results <- get_rep_cor_dfs(d.lfc_annot_adj_no_dups, "pgRNA_id", "lfc_late_vs_T0")
