...
d.pDNA <- d.counts_flagged_timepoint_named %>%
  filter(sample == "pDNA") %>%
  group_by(id) %>%
  summarize(pDNA_log2_cpm = mean(log2_cpm), .groups = "drop")

d.late_lfc <- d.counts_flagged_timepoint_named %>%
  dplyr::filter(timepoint == "late") %>%
  dplyr::select(id, sample, day, rep, count, late_log2_cpm = log2_cpm, low_pDNA_cpm, zero_count) %>%
  dplyr::left_join(d.pDNA, by = "id") %>%
  dplyr::mutate(lfc_late_vs_pDNA = late_log2_cpm - pDNA_log2_cpm)  
...
##lets calculate %CV here for each pgRNA 

d.lfc_annot.cv <- d.lfc_annot %>% 
  dplyr::filter(norm_ctrl_flag == "negative_control") %>%
  group_by(pgRNA_id) %>%
  summarise(mean_lfc = mean(lfc_late_vs_pDNA, na.rm = TRUE),
            sd_lfc = sd(lfc_late_vs_pDNA, na.rm = TRUE),
            .groups = "drop")

pgrna.rem.cv <- d.lfc_annot.cv %>% 
  filter(sd_lfc > 2) %>%
  pull(pgRNA_id)

d.removed_pgRNAs <- d.lfc_annot %>%
  dplyr::select(pgRNA_id, paralog_pair, low_pDNA_cpm) %>%
  distinct(pgRNA_id, .keep_all = TRUE) %>%
  filter(low_pDNA_cpm == TRUE) 
...
d.lfc_annot <- d.lfc_annot %>%
  filter(low_pDNA_cpm == FALSE)%>%
  filter(!pgRNA_id %in% pgrna.rem.cv)

d.lfc_annot

save_tbl(d.lfc_annot)

unadjusted_norm_ctrl_violin_plot <- make_norm_ctrl_violin_plot(d.lfc_annot, "lfc_late_vs_pDNA", "raw_LFC")
...
d.lfc_annot %>%
  ...
  summarize(mean = mean(lfc_late_vs_pDNA),
            sd = sd(lfc_late_vs_pDNA)) %>%
...
##Alice add NNMD
d.nnmd.avg.pgrna <- d.lfc_annot %>%
  group_by( pgRNA_id) %>%
  summarize(lfc_avg = mean(lfc_late_vs_pDNA, na.rm = TRUE),
            across( -lfc_avg, first),
            .groups="drop")

d.nnmd.avg.val.pgrna <- d.nnmd.avg.pgrna %>% 
  filter(norm_ctrl_flag == "negative_control" | norm_ctrl_flag == "positive_control") %>%
  group_by(norm_ctrl_flag) %>%
  summarize(median = median(lfc_late_vs_pDNA),
            mad = mad(lfc_late_vs_pDNA)) %>%
  pivot_wider(names_from = norm_ctrl_flag, 
              values_from = c(median, mad),
              names_glue = "{norm_ctrl_flag}_{.value}") %>%
  mutate(nnmd = (positive_control_median - negative_control_median) /
           negative_control_mad)
print(d.nnmd.avg.val.pgrna)
save_tbl(d.nnmd.avg.val.pgrna)

d.nnmd.val.pgrna <- d.nnmd.avg.val.pgrna$nnmd

nnmd.avg.dens.pgrna <- ggplot(subset(d.nnmd.avg.pgrna, norm_ctrl_flag %in% c("positive_control", "negative_control")),
                              aes(x =lfc_avg, fill = norm_ctrl_flag, color = norm_ctrl_flag)) +
  geom_density(alpha = 0.4) + xlim(-10, 4)  +
  labs(title = paste0("L2FC Density, NNMD = ", round(d.nnmd.val.pgrna, 3), ")"), x = "Log2 Fold Change (L2FC)", y = "Density") +
  theme_minimal() +
  theme(text = element_text(size = 12), legend.title = element_blank()) 

print(nnmd.avg.dens.pgrna)
save_plot(nnmd.avg.dens.pgrna)
...
##rep correlation after adjusting
d.lfc_annot_adj_no_dups <- d.lfc_annot_adj %>%
  distinct(pgRNA_id, rep, .keep_all = TRUE)

dim(d.lfc_annot_adj_no_dups)

results <- get_rep_cor_dfs(d.lfc_annot_adj_no_dups, "pgRNA_id", "lfc_late_vs_pDNA")

d.adj_lfc_rep_cor_plot <- results[[1]]
d.adj_lfc_rep_cor_summary <- results[[2]]


print_kbl(d.adj_lfc_rep_cor_summary)

## make replicate correlation plot
make_rep_cor_plot(d.adj_lfc_rep_cor_plot, d.adj_lfc_rep_cor_summary, "LFC")
save_plot(make_rep_cor_plot(d.adj_lfc_rep_cor_plot, d.adj_lfc_rep_cor_summary, "LFC"))
...
##lets do target-level NNMD here now

d.nnmd.avg.target <- d.lfc_annot_adj_target %>%
  summarize(lfc_avg = mean(lfc_late_vs_pDNA, na.rm = TRUE),
            across( -lfc_avg, first),
            .groups="drop")

d.nnmd.avg.val.target <- d.nnmd.avg.target %>% 
  filter(norm_ctrl_flag == "negative_control" | norm_ctrl_flag == "positive_control") %>%
  group_by(norm_ctrl_flag) %>%
  summarize(median = median(lfc_late_vs_pDNA),
            mad = mad(lfc_late_vs_pDNA)) %>%
  ...
  mutate(nnmd = (positive_control_median - negative_control_median) /
           negative_control_mad)
print(d.nnmd.avg.val.target)
save_tbl(d.nnmd.avg.val.target)

d.nnmd.val.target <- d.nnmd.avg.val.target$nnmd

nnmd.avg.dens.target <- ggplot(subset(d.nnmd.avg.target, norm_ctrl_flag %in% c("positive_control", "negative_control")),
                               aes(x =lfc_avg, fill = norm_ctrl_flag, color = norm_ctrl_flag)) +
  geom_density(alpha = 0.4) + xlim(-10, 4)  +
  labs(title = paste0("L2FC Density, NNMD = ", round(d.nnmd.val.target, 3), ")"),
       x = "Log2 Fold Change (L2FC)", y = "Density") +
  theme_minimal() +
  theme(text = element_text(size = 12), legend.title = element_blank()) 

print(nnmd.avg.dens.target)
save_plot(nnmd.avg.dens.target)
