...
d.pan_essential <- read_csv(file.path(in_dir, "depmap_common_essentials.csv"))
...
ensembl <- useEnsembl(biomart="ensembl", dataset="hsapiens_gene_ensembl") ## Alice had to update
...
d.pan_essential <- d.pan_essential %>%
  separate(col = Essentials, into = c("gene_symbol", "entrez_id"), remove = FALSE)
...
fake_gene_nums <- sample(
  rep(seq_len(n_fake_genes), length.out = nrow(d.annot_neg_ctrl_only))
)

d.annot_neg_ctrl_only <- d.annot_neg_ctrl_only %>%
  mutate(pgRNA_target = paste0("FAKE_GENE_", fake_gene_nums))
