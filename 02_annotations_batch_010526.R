...
my_depmap_id <- d.depmap_metadata %>%
  filter(grepl(cell_line, stripped_cell_line_name, ignore.case = TRUE)) %>%
  pull(depmap_id)

if(length(my_depmap_id) == 0){
  stop(paste0("No DepMap ID found for cell line: ", cell_line))
} else if(length(my_depmap_id) > 1){
  stop(paste0("Multiple DepMap IDs found for cell line: ", cell_line))
}
...
d.pgMI_gene_tpm_filtered_summary <- d.pgMI_gene_tpm_filtered %>%
  group_by(expressed_flag) %>%
  summarize(n = n())
print_kbl(d.pgMI_gene_tpm_filtered_summary)
...
## ok to ignore any genes w/ tpm = NA because we're just taking the median of
##   expressed vs. unexpressed gene - it's ok if not all of them have a TPM
d.pgMI_gene_cn_na_summary <- d.pgMI_gene_cn %>%
  filter(is.na(log2_cn))
print_kbl(d.pgMI_gene_cn_na_summary)

d.pgMI_gene_cn_duplicated <- d.pgMI_gene_cn %>%
  group_by(gene_symbol) %>%
  mutate(duplicated = n() > 1) %>%
  filter(duplicated == TRUE)
print_kbl(d.pgMI_gene_cn_duplicated)
