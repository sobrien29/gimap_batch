...
d.counts_cpm_T0 <- d.counts_cpm %>%
  filter(sample == paste0( cell_line,"Day00_RepA"))

## what does the T0 read count distribution look like? 
T0_cpm_histogram <-  ggplot(d.counts_cpm_T0, aes(x = log2_cpm)) +
...
T0_cpm_histogram_cutoff <- T0_cpm_histogram + 
  geom_vline(xintercept = T0_cpm_cutoff, ## adjust based on selected cutoff
             linetype = "dashed") 
T0_cpm_histogram_cutoff
save_plot(T0_cpm_histogram_cutoff)

## add filter variable to pgRNA df based on selected cutoff
d.counts_cpm_T0_filter <- d.counts_cpm_T0 %>%
  mutate(low_T0_cpm = case_when(
    log2_cpm < T0_cpm_cutoff ~ TRUE, ## if plasmid log2_cpm < cutoff, set to TRUE
    TRUE ~ FALSE)) %>% ## if above condition is not met, set to FALSE
  ungroup()

## how many guides will be removed based on this filter?
d.summ <- d.counts_cpm_T0_filter %>%
  dplyr::select(id, low_T0_cpm) %>%
  distinct(id, .keep_all = TRUE) %>%
  group_by(low_T0_cpm) %>%
  summarize(n = n()) %>%
  mutate(percent = round(((n/sum(n))*100), 2))
...
d.counts_cpm_filter <- d.counts_cpm_T0_filter %>%
  dplyr::select(id, low_T0_cpm) %>%
  right_join(d.counts_cpm_filter, by = "id") %>%
  dplyr::select(id, sample:zero_count, low_T0_cpm) %>% ## reorder cols
  ungroup()

## how many pgRNAs will be removed by both filters?
d.counts_cpm_filter <- d.counts_cpm_filter %>%
  dplyr::select(id, zero_count, low_T0_cpm) %>%
  distinct(id, .keep_all = TRUE) %>%
  group_by(id) %>%
  mutate(rm_pgRNA = case_when(
    any(zero_count == TRUE | low_T0_cpm == TRUE) ~ TRUE,
    TRUE ~ FALSE)) %>%
  ungroup()
...
d.summ_complete <- d.counts_cpm_filter %>%
  mutate(flag_group = case_when(
    zero_count == TRUE & low_T0_cpm == TRUE ~ "both",
    zero_count == TRUE & low_T0_cpm == FALSE ~ "zero_count_only",
    low_T0_cpm == TRUE & zero_count == FALSE ~ "low_plasmid_cpm_only",
    low_T0_cpm == FALSE & zero_count == FALSE ~ "neither",
    TRUE ~ "error"
  )) %>%
  group_by(flag_group) %>%
  summarize(n = n())
